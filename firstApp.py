...
lines1 = open("type1.txt", encoding='utf-8', errors='ignore').read().split('\n\n')
linest1=[]
for line in lines1:
    _line=line.split('\n')
    linest1.append(_line)
for row in linest1:
    del row[:2]

lines2 = open("type2.txt", encoding='utf-8', errors='ignore').read().split('\n')
linest2=[]
for line in lines2:
    _line=line.split('}')
    linest2.append(_line)
for row in linest2:
    del row[:2]

lines3 = open("type3.txt", encoding='utf-8', errors='ignore').read().split('\n')
linest3=list(filter(None,lines3))    
del linest3[1::2]
...
for line in movie_lines:
    _line = line.split(' +++$+++ ')
    if len(_line) == 5:
        linesst4[_line[0]] = _line[4]

linest4=linesst4.values()
linest4=list(linest4)


liness=linest1+linest2+linest3+linest4
